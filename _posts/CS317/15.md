...
repeat
	Use the union rule to replace any dependencies in Fc of the form
		alpha -> beta and alpha -> gamma with alpha -> beta gamma
Find a functional dependency alpha to beta in Fc with an extraneous attribute in alpha or in beta
...
![image-20220217224504065](assets/image-20220217224504065.png)
...
while(not done)
	if (there is a schema Ri in result not in 4NF)
		...
		on Ri such that  alpha to Ri is not in Di and alpha cap beta is null
		result = {result - Ri, Ri - beta, (alpha, beta)}
	else done = true
```
